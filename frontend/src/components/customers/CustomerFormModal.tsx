...
const EMPTY: CustomerFormValues = { name: '', company: '', email: '', phone: '', address: '', notes: '' }

export function CustomerFormModal({ open, onClose, onSubmit, initial, submitting }: Props) {
  const {
    register,
    handleSubmit,
    reset,
    formState: { errors },
  } = useForm<CustomerFormValues>({ resolver: zodResolver(customerSchema), defaultValues: EMPTY })
