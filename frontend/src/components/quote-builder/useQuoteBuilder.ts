...
import type { QuoteStatusDb, QuoteWithCustomer, QuoteItemRow } from '@/types/database'
import { newLineItem, type BuilderLineItem } from './LineItemsEditor'
...
function fromQuoteRow(quote: QuoteWithCustomer, items: QuoteItemRow[]): BuilderState {
  return {
...
export function useQuoteBuilder(params: {
  ...
  loaded?: { quote: QuoteWithCustomer; items: QuoteItemRow[] }
}) {
  ...
  const [status, setStatus] = useState<QuoteStatusDb>(loaded?.quote.status ?? 'DRAFT')
  ...
  const [state, setState] = useState<BuilderState>(() => {
    ...
    if (saved) {
      try {
        return JSON.parse(saved) as BuilderState
      } catch {
  ...
  const performSave = useCallback(
    async (nextStatus?: QuoteStatusDb) => {
      ...
      const result = await saveQuote.mutateAsync({
        ...
        items: state.items.map((item, index) => ({
          ...
          tax_percent: item.tax_percent,
          line_total: Number(
            calculateQuoteTotals({ items: [item] }).total
          ),
          sort_order: index,
        })),
      })
